...
def find_available_slots(serialized_agenda, duration)
  available_slots = []
  current_time = serialized_agenda.first['start'].to_date

  until current_time == serialized_agenda.last['end'].to_date.next
    start_of_day = DateTime.new(current_time.year, current_time.month, current_time.day, 9, 0, 0)
    end_of_day = DateTime.new(current_time.year, current_time.month, current_time.day, 18, 0, 0)
    available_slots.concat(find_slots_for_day(serialized_agenda, start_of_day, end_of_day, duration))
    current_time = current_time.next
  end
  available_slots
end

def find_slots_for_day(events, start_of_day, end_of_day, duration)
  available_slots = []
  ...
  events.each do |event|
    ...
    next if start_time >= end_of_day || end_time <= start_of_day
